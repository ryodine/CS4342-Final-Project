...
train, validate, test = get_data_generators(traincsv="../balanced_4000_train.csv", testcsv="../TEST_DATA_SPLIT.csv",
                                            validationcsv="../VALIDATION_DATA_SPLIT.csv", imdir="../train_images")

models = [get_model_2(), get_model_4(), get_model_5(),
          get_model_5_1(), get_model_6()]


def predict_raw_consensus(data_gen):
    predictions = [np.argmax(model.predict(data_gen), axis=1)
                   for model in models]
